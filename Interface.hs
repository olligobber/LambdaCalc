...
interpretTimes :: Maybe Int -> Expression -> (Int, Expression)
interpretTimes (Just 0) x = (0, x)
interpretTimes n x = case simplify x of
    Nothing -> (0, x)
    Just y -> (t+1, z) where
        (t,z) = interpretTimes (pred<$>n) y
